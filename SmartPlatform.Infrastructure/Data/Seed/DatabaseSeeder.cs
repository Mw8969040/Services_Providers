    ...
    public static class DatabaseSeeder
    {
        public static async Task SeedAllAsync(ApplicationDbContext context, UserManager<ApplicationUser> userManager)
        {
            ...
            try
            {
                ...
                {
                    if (!await context.ServiceCategories.AnyAsync(c => c.Name == catName))
                    {
                        await File.AppendAllTextAsync(logPath, $"Seeding Category: {catName}\n");
                        var desc = catName switch
                        {
                            "Plumbing" => "Professional plumbing and pipe repair services.",
                            "Electrical" => "Electrical wiring, repairs, and appliance maintenance.",
                            "Cleaning" => "Home, office, and deep cleaning services.",
                            _ => ""
                        };
                        var img = catName switch
                        {
                            "Plumbing" => "https://images.unsplash.com/photo-1585704032915-c3400ca199e7?auto=format&fit=crop&q=80&w=800",
                        ...
                        await context.ServiceCategories.AddAsync(new ServiceCategory { Name = catName, Description = desc, ImageUrl = img });
                    }
                }
                ...
                foreach (var prov in providerUsers)
                {
                    ...
                    {
                        await File.AppendAllTextAsync(logPath, $"Creating Provider: {prov.Email}\n");
                        var result = await userManager.CreateAsync(prov, "Password@123");
                        if (result.Succeeded)
                        {
                ...
                foreach (var cust in customerUsers)
                {
                    ...
                    {
                        await File.AppendAllTextAsync(logPath, $"Creating Customer: {cust.Email}\n");
                        var result = await userManager.CreateAsync(cust, "Password@123");
                        if (result.Succeeded)
                        {
                ...
                if (ahmed != null && sara != null)
                {
                    ...
                    if (plumbingCat != null && cleaningCat != null)
                    {
                        ...
                        {
                            if (!await context.Services.AnyAsync(s => s.Title == service.Title))
                            {
                                await File.AppendAllTextAsync(logPath, $"Seeding Service: {service.Title}\n");
                ...
                if (omar != null && ahmed != null)
                {
                    ...
                    if (sinkService != null && !await context.ServiceRequests.AnyAsync(r => r.CustomerId == omar.Id && r.ServiceId == sinkService.Id))
                    {
                        await File.AppendAllTextAsync(logPath, "Seeding Sample Request & Review...\n");
                ...
                await File.AppendAllTextAsync(logPath, "Seeding Completed Successfully.\n");
            }
            catch (Exception ex)
            {
                await File.AppendAllTextAsync(logPath, $"CRITICAL SEEDING ERROR: {ex.Message}\n{ex.StackTrace}\n");
            }
        }
    }
}
